...
import type { IndexedNote } from "./indexer";
...
export class FilterView extends ItemView {
    ...
    render() {
        ...
        Object.values(index).forEach(note => {
            ...
            if (note.concepts.studyType) {
                ...
                if (key) studyTypeCounts[key] = (studyTypeCounts[key] || 0) + 1;
            }
        });
    ...
    renderSection(
        ...
        category: "models" | "endpoints" | "techniques" | "studyTypes",
    ...
    renderMatchingNotes(container: HTMLElement) {
        ...
        const matches = Object.entries(this.plugin.vectorIndex)
            .filter(([_, data]) => {
                ...
                const matchCategory = (set: Set<string>, values?: unknown[]) => {
                    if (set.size === 0) return true;
                    const normalizedValues = normalizeArray(values);
                    return [...set].every(v => normalizedValues.includes(v));
                };

                const matchStudyType = (set: Set<string>, value?: unknown) => {
                    if (set.size === 0) return true;
                ...
                return (
                    matchCategory(activeFilters.models, models) &&
        ...
        if (Object.values(activeFilters).every(set => set.size === 0)) return;
    ...
    renderInsightButton(container: HTMLElement) {
    ...
    async generateInsights(container: HTMLElement) {
        const normalize = (v: unknown): string | null => {
            if (typeof v !== "string") return null;
            const cleaned = v.trim().toLowerCase();
        ...
        const matches = Object.entries(this.plugin.vectorIndex)
            .filter(([_, data]) => {
                ...
                const matchCategory = (set: Set<string>, values: string[]) => {
                    if (set.size === 0) return true;
                    return [...set].every(v => values.includes(v));
                };

                const matchStudyType = (set: Set<string>, value: string | null) => {
                    if (set.size === 0) return true;
                ...
                return (
                    matchCategory(this.selectedFilters.models, models) &&
        ...
        const summary = matches.map(([path, data]) => ({
            ...
            doses: data.parsedDoses?.map((d) =>
                `${d.totalGy} Gy / ${d.fractions} Fx`
            )
        }));
        ...
        const response = await fetch("http://localhost:11434/api/generate", {
            ...
            body: JSON.stringify({
                model: this.plugin.settings.insightModel,
                stream: false,
                prompt: `
...
`
    ...
    showMatchingNotes(category: string, value: string) {
        const matches = Object.entries(this.plugin.vectorIndex)
            .filter(([_, data]) => {
                if (!data.concepts) return false;
